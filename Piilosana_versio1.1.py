...
def kur_siirto_seur(ruudussa, ind, lkm, x, y):
    ...
    if ind == len(ruudut)-1:
        ...
        if not (ruudut[ind+1].musta or ruudut[ind+1].kirjain or x == lkm):
            return [x+1,y]
        elif (x < lkm-1):
...
# Vihjeet ruudulle
def vihjeet(lkm):
    # pyyhitään sanaehotukset pois
    pygame.draw.rect(screen,WHITE,[lkm*30+60,110,lkm*30,10*25])
    # Vihjelista
    vihjeet = ['Vaakasuoraan:','1. ()'\
    ,'2. ()','3. ()'\
    ,'4. ()'\
    ,'5. ()','6. ()'\
    ,'7. ()','8. ()'\
...
def autosana(vir_x, vir_y, kur, xy_lkm, suomensanat):
    ...
    print(ind)
    while not (vir_x > xy_lkm or vir_y > xy_lkm or ind > xy_lkm*xy_lkm - 1): # kunnes vastaan
        # tulee ruudukon reuna
        if kur == 'oikea' and vir_x == xy_lkm:
            break
        if ruudut[ind].kirjain: # jos ruudussa kirjain, tallentaan kirjaimet-hakulistaan
            kirjaimet[kir_lkm] = ruudut[ind].kirjain 
        elif ruudut[ind].musta: # stopataan, jos musta ruutu
            break
        vir_x += seur[0]
        vir_y += seur[1]
        kir_lkm += 1
        ind += seur[0]+seur[1]*xy_lkm

    print(kirjaimet, kir_lkm)
...
def main(ruudut, xy_lkm):
    ...
    merkit = ["A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P","Q",
                ...
                "8","9",'musta','oikea','alas','pois','sanaehdotus','vihjeet']
    keys = [pygame.K_a,pygame.K_b,pygame.K_c,pygame.K_d,pygame.K_e,
        ...
        pygame.K_RIGHT,pygame.K_DOWN,pygame.K_BACKSPACE,pygame.K_SPACE, pygame.K_RETURN]
    ...
    # Ohjeet ruudulle
    ohjeet = ['Kursorin asento: NUOLI-nääpäimet oikealle/alas','Skandit: klikkaa A:n tai O:n päällä','Musta ruutu: SHIFT-oikea','Sanaehdotus: SPACE','Vihjeet ruudulle: RETURN']
    count = 0
    for ii in ohjeet:
        rivi = tyyli(16,'Courier').render(ii, True, BLACK)
        screen.blit(rivi, [20, 5 + count*18])
        count += 1
    ...
    # Hiiren klikkaus loppuu --- Näppäin alas alkaa --------------------
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if ekey in keys:
                    ...
                    elif indeksi == 41: # RETURN -> vihjeet näkyviin
                        vihjeet(xy_lkm)
